 ...
 * schema (see packages/database/migrations/); these tests do not run the
...
export function getIntegrationDb(): DatabaseClient {
  ...
  if (!databaseUrl) {
    throw new Error(
      'DATABASE_URL is not set — see docs/13-roadmap/SPRINT-1-COMPLETION-REPORT.md for how to run this suite.',
    );
  }
