import { z } from 'zod';
import { cancelEstimate } from '@atlas/financials';
...
/** POST /api/v1/estimates/{id}/cancel — estimates.md#state-machine: `draft`/`sent` -> `cancelled`. Accountant/Admin/Owner only (`estimates:void`). */
...
export const POST = withApiHandler<unknown, { params: { id: string } }>(
  async (request, context) => {
    ...
    const estimate = await cancelEstimate(getDb(), {
