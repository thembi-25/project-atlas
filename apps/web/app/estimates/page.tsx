...
import type { EstimateDto } from '@/lib/financials-types';

/**
 * Estimate list — estimates-prd.md. No global app shell/organization
 * switcher exists yet, same known limitation as every other Sprint 2-4
 * list page — the active Organization is read from `?organization_id=`.
 */
...
function EstimatesPageContent(): JSX.Element {
  ...
  const load = useCallback(
    async (opts: { cursor?: string | null; append?: boolean } = {}) => {
      ...
      try {
        ...
        const result = await apiRequest<EstimateDto[]>(`/api/v1/estimates?${params.toString()}`);
        setEstimates((prev) => (opts.append ? [...prev, ...result.data] : result.data));
    ...
    return (
      <main className="mx-auto max-w-4xl p-8">
        <p className="text-muted-foreground text-sm">
          ...
          the URL to view Estimates for an Organization.
        </p>
      </main>
    );
  }
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {estimates.map((estimate) => (
            <tr key={estimate.id} className="border-border border-b">
              <td className="py-2">
                <Link
                  className="hover:underline"
                  href={`/estimates/${estimate.id}?organization_id=${organizationId}`}
                >
                  {estimate.estimate_number}
                </Link>
              </td>
              <td className="py-2 capitalize">{estimate.status}</td>
              <td className="py-2">${estimate.total}</td>
              <td className="py-2">
...
function CreateEstimateForm({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      await apiRequest('/api/v1/estimates', {
        method: 'POST',
        body: JSON.stringify({
