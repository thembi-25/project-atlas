...
Accepted (design decision for a future phase — see [Roadmap Phase 4](../13-roadmap/phase-4-marketplace.md))
...
The Supplier Marketplace (Vision pillar 3 — see [Vision](../00-overview/vision.md)) is a Phase 4 capability, not launch scope, but its data-model shape needs to be decided now so the launch-scope [Inventory](../03-domain/inventory.md) and [Suppliers](../03-domain/suppliers.md) modules aren't built in a way that has to be reworked when Marketplace arrives.
...
2. **Building Marketplace now, ahead of Phase 4, to avoid future rework** — rejected. Contradicts the explicit phase-gating rationale in [Product Strategy](../01-product/product-strategy.md): Marketplace requires a credible base of active Organizations before Supplier partnerships are viable, and building the infrastructure before that base exists is premature investment against an unvalidated assumption.
...
- When Phase 4 begins, `marketplace_listings` and `marketplace_orders` are additive tables referencing existing `inventory_items`/`suppliers`, not a schema redesign.
...
- Real-world Supplier catalogs may not map cleanly 1:1 onto Atlas's `inventory_items` taxonomy (a Supplier's SKU granularity may differ) — this is a genuine future design challenge, tracked as an open question for Phase 4 design work rather than resolved prematurely now.

## Migration / Rollback

Not applicable at this stage — no Marketplace code exists yet. This ADR constrains *how* Phase 4 implementation begins, not a system already built and needing to be unwound.

## Related Decisions

[Marketplace](../03-domain/marketplace.md) · [Marketplace PRD](../06-modules/marketplace-prd.md) · [Inventory](../03-domain/inventory.md) · [Roadmap Phase 4](../13-roadmap/phase-4-marketplace.md)
